...
@Composable
fun PulihkanSandi(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Text(
            text = "Lupa kata sandi? Jangan khawatir, masukkan email Anda untuk mengatur ulang kata sandi Anda saat ini.",
            style = TextStyle(
                fontFamily = rubikfamily,
        ...
        val annotatedText = buildAnnotatedString {
            ...
            // Tambahkan anotasi untuk bagian teks "Daftar"
            pushStringAnnotation(tag = "DAFTAR", annotation = "daftar")
            withStyle(style = SpanStyle(color = Color(0xFF33A506), fontWeight = FontWeight.Bold)) {
                append("Daftar")
            }
            pop()
        }
        ...
        text = annotatedText,
        onClick = { offset ->
            annotatedText.getStringAnnotations(tag = "DAFTAR", start = offset, end = offset)
                .firstOrNull()?.let {
                    onClickDaftar()
                }
        },
        style = TextStyle(fontSize = 14.sp, color = Color.Black, fontFamily = rubikfamily, fontWeight = FontWeight.Medium) // warna default teks
    )
...
@Composable
private fun PulihkanSandiPreview() {
    PulihkanSandi()
